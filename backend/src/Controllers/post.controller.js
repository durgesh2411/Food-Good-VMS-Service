...
const createPost = asyncHandler(async (req, res) => {
  ...
  // Allow volunteers and admins to create posts
  if (!(req.user.role === "volunteer" || req.user.isAdmin)) {
    throw new ApiError(
      400,
      "You do not have permission to create a post. Only volunteers and admins can create posts."
    );
  }
  ...
  // Auto-approve admin posts, volunteers need approval
  ...
  const message = req.user.isAdmin
    ...
    : "Post created successfully and is pending admin approval.";

  return res.status(201).json(new ApiResponse(200, post, message));
});
...
const rejectPost = asyncHandler(async (req, res) => {
  ...
  if (!req.user.isAdmin) {
    throw new ApiError(400, "You do not have permission to reject posts");
  }
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, post, "Post rejected successfully"));
});
