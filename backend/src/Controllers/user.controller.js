...
const registerUser = asyncHandler(async (req, res) => {
  ...
  console.log("Extracted fields:", { fullName, email, number, role });
  ...
  if (existedUser) {
    ...
    // Check which field is duplicate for better error message
    const duplicateEmail = await User.findOne({ email });
    const duplicateNumber = await User.findOne({ number });

    if (duplicateEmail && duplicateNumber) {
      throw new ApiError(
        409,
  ...
  console.log("Created user found:", createdUser ? "Yes" : "No");
...
// Temporary function to make user admin for testing
